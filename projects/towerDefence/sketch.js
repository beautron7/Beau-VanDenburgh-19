...
var Grid = {
	...
	damageMap: [],
	/*
	damageMap is a variable which helps match the location of runners to what towers can hit them
	...
	, sees if they (Towers) have shot anything before and what their target limit is, and then changes its (runners) healh.
	this prevents runners from checking each and every tower, and tower's from checking each and every runner,
	which increases the framrate

	* if a tile collides with minrange, it will not be fired at. this is sacraficed due to library limitations of p5collide.js.
	*/
	routeHopsToEnd: [],
	/*
	...
	*/
	...
	verifyRoute: function(array_to_verify) {//used to check to see if the route generated by generateRoute leads to spawns (hint: it does unless you intentionaly block it)
		...
		{
			if (array_to_verify[this.spawns[i].x][this.spawns[i].y] >= 2048)
			{ //basically, if the distance is over 2048, its not vaid, so return false
...
function kill_stuck_runners(seconds_old){//brokenish; occasionally, runners would try and go to the edge and stay there.
	for(var i = 0; i < runners.length; i++){
